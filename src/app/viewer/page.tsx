...
function RotatingModel({ modelUrl, isPaused }: { modelUrl: string; isPaused: boolean }) {
  ...
  // Rotate the model only if not paused
  useFrame(() => {
    if (modelRef.current && !isPaused) {
      modelRef.current.rotation.y += 0.002; // Adjust the value for speed
    }
...
export default function Viewer() {
  ...
  const [selectedGrade, setSelectedGrade] = useState<string | null>("C Grade"); // Default selected
  const router = useRouter();
  const timeoutRef = useRef<NodeJS.Timeout | null>(null);

  useEffect(() => {
    // On mount, load the model URL from query params (Cloudinary) if available.
    const urlParams = new URLSearchParams(window.location.search);
    const modelUrlFromQuery = urlParams.get("modelUrl");
    if (modelUrlFromQuery) {
      setModelUrl(modelUrlFromQuery);
  ...
  // Handle file selection: update the modelUrl with a temporary object URL.
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: "24px" }}>
        ...
        >
          <Canvas gl={{ antialias: true }} dpr={[0.3, 3.5]} camera={{ position: [4, -1, 8], fov: 35 }}>
            <color attach="background" args={["#f5f5f5"]} />
            <Suspense
              fallback={
                <Html center>
            ...
            >
              ...
              <Stage   intensity={0} adjustCamera={1.2} preset="rembrandt" environment={"city"} >
                {modelUrl && <RotatingModel modelUrl={modelUrl} isPaused={isPaused} />}
                <ambientLight  intensity={1} />  // {/*30 for iphone*/}
                {/* <directionalLight position={[0, 1, 0]} intensity={2} />  Top light */}
{/* <directionalLight position={[-1, 0, 0]} intensity={20} /> Left light */}
{/* <directionalLight position={[1, 0, 0]} intensity={5} />  Right light */}
{/* <directionalLight position={[1, -1, 0]} intensity={5} />  //  bottom side  //   30 for iphone */}
{/* <directionalLight position={[0, 0, 1]} intensity={0} />    Back light */}
{/* <ambientLight position={[0, 0, -1]} intensity={20} />  Front light */}
        ...
        {/* Right Container - Grades List and Upload Button */}
        <div
          style={{
            width: "325px",
            height: "460px",
